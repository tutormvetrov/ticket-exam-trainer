...
- `PASS` означает, что отказ обработан честно
...
| Библиотека / Import UI | импортируемый файл повреждён или формат не поддерживается | вызвать `open_import_dialog()` с неподдерживаемым путём | должен появиться честный error dialog без ложного успеха | `QMessageBox.critical` вызван с текстом об unsupported format | человекочитаемая ошибка surfaced | PASS | `tests/test_ui_handlers.py` |
