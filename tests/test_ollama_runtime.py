...
def _prepare_models_path(path: Path) -> None:
    (path / "manifests" / "registry.ollama.ai" / "library" / "mistral").mkdir(parents=True, exist_ok=True)
    (path / "manifests" / "registry.ollama.ai" / "library" / "mistral" / "instruct").write_text("manifest", encoding="utf-8")
